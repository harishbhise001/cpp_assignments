    ...
    while(( choice = menu() ) != EXIT)
    {
        switch(choice)
        {
            case ACCEPT:
            {
                ...
                if(ch == 1) emp.push_back(new Manager());
                else if(ch == 2) emp.push_back(new Salesman());
                else if(ch == 3) emp.push_back(new SalesManager());

                if(emp[emp.size()-1] != NULL) emp[emp.size()-1]->accept();
                break;
            }
